...
public final class SnakeApp extends JFrame {
    ...
    public SnakeApp() {
        ...
        var exec = Executors.newCachedThreadPool();        snakes.forEach(s -> exec.submit(new SnakeRunner(s, board, pauseController)));
        ...
        for (Snake snake : snakes) {
            if (snake.isAlive()) {
                if (longestAlive == null ||
                    snake.length() > longestAlive.length()) {
                    longestAlive = snake;
                }
            }
            ...
            if (!snake.isAlive()) {
                if (firstDead == null ||
                    snake.deathOrder() < firstDead.deathOrder()) {
                    firstDead = snake;
                }
            }
        }

        String longestMessage = longestAlive == null
                ? "No hay serpientes vivas."
                : "Serpiente viva más larga: " + longestAlive.length();

        String firstDeadMessage = firstDead == null
                ? "Ninguna serpiente ha muerto."
                : "Primera serpiente en morir: " + firstDead.deathOrder();
